...
file_path = "./finalSpotify/final_df.csv"
...
def plot_single_feature_label(data: data_wrapper, rows, feature, size=5):
    ...
    for i in range(rows):
        ...
        feature_v = data.get_feature_values_2(song_data)[0]
        x.append(feature_v)

        label = songs.get_label_2(song_data)
        y.append(label)
...
def plot_two_features_label(data: data_wrapper, rows, f1, f2):
    ...
    for i in range(rows):
        song_data = data.get_row(i)
        f1_value = data.get_feature_values_2(song_data)[0]
        f2_value = data.get_feature_values_2(song_data)[1]
        x.append(f1_value)
        y.append(f2_value)
        l = songs.get_label_2(song_data)
        if (l == 1):
            clrs.append('green')
        else:
            clrs.append('red')
...
plt.figure('valence')
plot_single_feature_label(songs, plot_row, ['valence'])
