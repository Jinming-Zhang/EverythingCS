...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # visited corner indicator in clock-wise order starting from top-left
        self.start = (self.startingPosition, 0, 0, 0, 0)
        self.top_left = (1, top)
        self.top_right = (right, top)
        self.bottom_right = (right, 1)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            pos, top_left, top_right, bottom_right, bottom_left = state
            x, y = pos
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            if (not self.walls[nextx][nexty]):
                next_pos = (nextx, nexty)
                if (next_pos == self.top_left):
                    top_left = 1
                if (next_pos == self.top_right):
                    top_right = 1
                if (next_pos == self.bottom_right):
                    bottom_right = 1
                if (next_pos == self.bottom_left):
                    bottom_left = 1

                successors.append(
                    (((nextx, nexty), top_left, top_right, bottom_right, bottom_left), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    bl_corner, tl_corner, br_corner, tr_corner = corners
    pos, top_left, top_right, bottom_right, bottom_left = state
    x, y = pos
    total_m_d = []
    if top_left == 0:
        total_m_d.append(abs(x-tl_corner[0]) + abs(y-tl_corner[1]))
    if top_right == 0:
        total_m_d.append(abs(x-tr_corner[0]) + abs(y-tr_corner[1]))
    if bottom_right == 0:
        total_m_d.append(abs(x-br_corner[0]) + abs(y-br_corner[1]))
    if bottom_left == 0:
        total_m_d.append(abs(x-bl_corner[0]) + abs(y-bl_corner[1]))
    if len(total_m_d) > 0:
        return max(total_m_d)
    else:
        return 0
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    food_poss = foodGrid.asList()
    dst = 0
    pos = position
    for food_pos in food_poss:
        dst += util.manhattanDistance(pos, food_pos)
        pos = food_pos
    return dst/2
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        return search.general_search(problem, search.SearchStrategy.BFS)
